...
public class EdgeConvertGUI {
    ...
    private void getOutputClasses() {
        ...
        if(outputDir == null){
            System.out.println("No output directory set");
        } else {
            ...
            try {
                for (int i = 0; i < resultFiles.length; i++) {
                    ...
                    if (resultClass.getSuperclass().getName().equals("EdgeConvertCreateDDL")) { //only interested in classes that extend EdgeConvertCreateDDL
                        ...
                        System.out.println("EdgeConvertGUI (994) - Object should be CreateDDLMySQL: " +objOutput.toString());
    ...
    class CreateDDLButtonListener implements ActionListener {
        public void actionPerformed(ActionEvent ae) {
            boolean loopFix = false;
            while (outputDir == null && !loopFix) {
            ...
            if (sqlString.equals(EdgeConvertGUI.CANCELLED) || sqlString.equals("")) {
                return;
            }
            writeSQL(sqlString);
        }
